...
class ApplyServiceTest {
     ...
     * 기본적으로 RedisConnectionFactory, RedisTemplate, StringRedisTemplate가 자동으로 구성되며 localhost:6379 주소의
    ...
    /**
     * 1. DB Lock을 이용하면 서비스의 성능이 느려지기 때문에 Redis를 이용해야한다 (Redis는 인메모리 DB이므로 속도의 측면에서 유리하다)
     * 이유는 아래와 같다.
     *    10:00 lock 획득
     *    10:01 쿠폰개수 획득
     *    10:02 쿠폰 생성
     ...
     * DB Lock 보다 성능이 뛰어나다는 장점을 가지고 있다. (redis는 O(1)의 시간복잡도를 갖는다.) & (pub/sub 시스템으로 이루어져있다.)
     *
    ...
    /**
     * 2. 위의 문제를 해결하기 위해 kafka의 도입이 필요하다. (implementation 'org.springframework.kafka:spring-kafka' 의존성 추가 필요)
     ...
     *
     * (쿠폰 로직 토픽(coupon_create) 생성) docker exec -it kafka kafka-topics.sh --bootstrap-server localhost:9092 --create --topic coupon_create
    ...
    public void 동시에여러명이응모() throws InterruptedException { // fail (첫 방법으로는 race condition 발생, redis를 이용할 경우 success)
        ...
        ExecutorService executorService = Executors.newFixedThreadPool(32); // 멀티 스레드 생성
